...
This directory vendors `terraform-aws-modules/eks/aws` version 20.37.2 from
the resolved Terraform source. It retains the upstream license and nested
modules. The only repository patch is the `aws_iam_session_context.current`
count in `main.tf`, gated by `enable_cluster_creator_admin_permissions` to
support AWS Academy sessions that cannot call `iam:GetRole`.
